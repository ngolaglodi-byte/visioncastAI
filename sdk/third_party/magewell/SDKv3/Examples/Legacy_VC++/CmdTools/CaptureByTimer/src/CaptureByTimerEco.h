...
void CaptureByTimerEco(HCHANNEL t_h_channel,LONGLONG t_ll_duration)
{
	...
	t_eco_capture_open.llFrameDuration = t_ll_duration; // when set -1,capture by input

	Bitmap bitmap(t_eco_capture_open.cx, t_eco_capture_open.cy, PixelFormat24bppRGB);
	...
	if (bitmapData.Stride < 0) {
		bitmapData.Scan0 = ((LPBYTE)bitmapData.Scan0) + bitmapData.Stride * (t_eco_capture_open.cy - 1);
