#ifndef CAPTUREBYINPUTECO_H
#define CAPTUREBYINPUTECO_H
...
void CapturePFrameEco(HCHANNEL t_h_channel)
{
	...
	if(t_video_signal_status.state != MWCAP_VIDEO_SIGNAL_LOCKED){
		printf("error:video signal invalid\n");
		return;
	}
	...
	t_eco_capture_open.cx = t_video_signal_status.cx;
	t_eco_capture_open.cy = t_video_signal_status.cy;
	t_eco_capture_open.dwFOURCC = MWFOURCC_BGR24;
	t_eco_capture_open.hEvent = t_h_event_capture;
	t_eco_capture_open.llFrameDuration = -1; // when set -1,capture by input

	Bitmap bitmap(t_video_signal_status.cx, t_video_signal_status.cy, PixelFormat24bppRGB);
	Rect rect(0, 0, t_video_signal_status.cx, t_video_signal_status.cy);
	...
	if (bitmapData.Stride < 0) {
		bitmapData.Scan0 = ((LPBYTE)bitmapData.Scan0) + bitmapData.Stride * (t_video_signal_status.cy - 1);
		bitmapData.Stride = -bitmapData.Stride;
		bBottomUp = TRUE;
	}

	MWCAP_VIDEO_ECO_CAPTURE_FRAME t_video_frame[3]={0};
	int t_n_stride = FOURCC_CalcMinStride(MWFOURCC_BGR24,t_eco_capture_open.cx,4);
	int t_n_size = FOURCC_CalcImageSize(MWFOURCC_BGR24,t_eco_capture_open.cx,t_eco_capture_open.cy,t_n_stride);
	for(int i=0;i<sizeof(t_video_frame)/sizeof(t_video_frame[0]);i++){
		t_video_frame[i].bBottomUp = bBottomUp;
		t_video_frame[i].cbFrame = t_n_size;
		t_video_frame[i].cbStride = t_n_stride;
		t_video_frame[i].deinterlaceMode = MWCAP_VIDEO_DEINTERLACE_WEAVE;
	...
	do 
	{
		...
		for(int i=0;i<3;i++)
			t_mr = MWCaptureSetVideoEcoFrame(t_h_channel,&(t_video_frame[i]));
		...
		for(int i=0;i<NUM_CAPTURE_ECO;i++){
			DWORD t_dw_ret = WaitForSingleObject(t_h_event_capture,INFINITE);
			if(t_dw_ret!=WAIT_OBJECT_0)
				break;

			MWCAP_VIDEO_ECO_CAPTURE_STATUS t_status_capture;
			while(i<NUM_CAPTURE_ECO){
				t_mr = MWGetVideoEcoCaptureStatus(t_h_channel,&t_status_capture);
				if(t_mr !=MW_SUCCEEDED || t_status_capture.pvFrame == NULL)
					break;

				memcpy(bitmapData.Scan0,t_status_capture.pvFrame,bitmapData.Stride*bitmapData.Height);

				t_mr = MWCaptureSetVideoEcoFrame(t_h_channel,(MWCAP_VIDEO_ECO_CAPTURE_FRAME*)t_status_capture.pvContext);
			}
		}
		...
		sprintf_s(path, "%s\\CaptureByInputEco.bmp", path);
