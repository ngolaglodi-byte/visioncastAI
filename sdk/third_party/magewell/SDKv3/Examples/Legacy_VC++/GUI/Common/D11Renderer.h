...
public:
	BOOL Init(int cx, int cy, DWORD dwFourcc, HWND hWnd) {
		...
		D3D_FEATURE_LEVEL    featureLevel = D3D_FEATURE_LEVEL_11_0;//D3D_FEATURE_LEVEL_11_0;
		...
		do {
			...
			hr = m_pDevice->CreateRenderTargetView(m_pBackBuffer, NULL, &m_pRenderView);
			if (FAILED(hr))
				break;
			...
			descDSV.Flags				= 0;
			hr = m_pDevice->CreateDepthStencilView(pDepthStencil, &descDSV,&m_pDepthView);
			if (FAILED(hr))
				break;
			...
			m_pImmediateContext->OMSetRenderTargets(1, &m_pRenderView, m_pDepthView); 
			...
			hr = m_pDevice->CreateQuery(&pQueryDesc, &m_pEventQuery);
			if (FAILED(hr))
				break;
	...
	BOOL CreateTexture(int nCount, const void* pSysMem) {
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			HRESULT hr = m_pDevice->CreateTexture2D( &desc_texture, &InitialData_texture, &m_arrTexture[i] );	
			if (FAILED(hr))
				return FALSE;
		}
	...
	ID3D11Resource* GetResource(int nIndex) {
		return m_arrTexture[nIndex];
	}

	BOOL Render(int nIndex) {
		m_pImmediateContext->CopyResource(m_pBackBuffer, m_arrTexture[nIndex]);
		m_pSwapChain->Present(0, 0);
		return TRUE;
	}
		
	BOOL Update(const void * pData, int cbStride) {
		m_pImmediateContext->UpdateSubresource(m_pBackBuffer, 0, NULL, pData, cbStride, 0);
