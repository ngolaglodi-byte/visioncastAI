...
bool MWRender::get_vsfs(s_csvs *t_vs,s_csvs *t_ps,DWORD t_dw_fourcc){
	bool t_ret=false;
...
HRESULT MWRender::compile_shader_from_file(WCHAR *t_sz_filepath,LPCSTR t_sz_entrypoint,LPCSTR t_sz_shader_model,ID3DBlob** t_pp_blob_out)
{
...
HRESULT MWRender::compile_shader_from_cs(unsigned char*t_p_data, int t_n_data_len, LPCSTR t_sz_entrypoint, LPCSTR t_sz_shader_model, ID3DBlob** t_pp_blob_out) {
	HRESULT t_hr = S_FALSE;
...
bool MWRender::create_render_with_fourcc(DWORD t_dw_fourcc)
{
	HRESULT t_hr=S_FALSE;
	...
	if(t_dw_fourcc==MWFOURCC_BGRA||t_dw_fourcc==MWFOURCC_BGR24){
...
void MWRender::destory_render_with_fourcc(DWORD t_dw_fourcc)
{

}

bool MWRender::initialize(int t_n_width,int t_n_height,DWORD dwFourcc,HWND t_hwnd)
{
	bool t_b_ret=false;
	m_b_set = false;
	HRESULT t_hr=S_FALSE;
	...
	create_render_with_fourcc(dwFourcc);

	return m_b_set;
}

void MWRender::cleanup_device()
{
...
void MWRender::paint(unsigned char*data)
{
	//clear buffer
	if (m_b_set == false)
		return;
