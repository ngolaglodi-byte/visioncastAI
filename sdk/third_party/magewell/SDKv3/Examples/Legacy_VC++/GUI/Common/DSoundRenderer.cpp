							 ...
							 int nSamplesPerFrame, int nFrameCount)
{
	...
	m_wfx.nAvgBytesPerSec = nSamplesPerSec * nChannels * m_wfx.wBitsPerSample / 8;
	...
	nFrameCount = (nFrameCount * m_cbFrameSize + HARDWARE_PLAYBACK_BUFFER_SIZE + m_cbFrameSize - 1) / m_cbFrameSize;
	...
	for (int i = 0; i < nFrameCount; i++) {
		dsPosNotify[i].dwOffset = i * m_cbFrameSize + (m_cbFrameSize - 1);
	...
	if (FAILED(m_lpDS->SetCooperativeLevel(hwnd, DSSCL_PRIORITY)) 
		...
		|| FAILED(m_lpDSBuf->QueryInterface(IID_IDirectSoundNotify, (LPVOID *)&m_lpDSNotify))
		|| FAILED(m_lpDSNotify->SetNotificationPositions(nFrameCount, dsPosNotify))
...
BOOL CDSoundRenderer::GetVolume(LPLONG lpVolume)
{
	return (SUCCEEDED(m_lpDSBuf->GetVolume(lpVolume)));
}
...
BOOL CDSoundRenderer::WriteWaveform(BOOL bSilence)
{
	...
	hr = m_lpDSBuf->Lock(m_dwWritePos, cbAvail, 
						 &pbData1, &cbData1, 
						 &pbData2, &cbData2, 0);
