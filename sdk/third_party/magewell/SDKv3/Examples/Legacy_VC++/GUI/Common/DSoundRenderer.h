...
public:
	BOOL Create(int nSamplesPerSec, int nChannels, 
				int nSamplesPerFrame, int nBufferFrameCount);
	void Destroy();
	...
	BOOL GetVolume(LPLONG lpVolume);
...
protected:
	...
	void GetFrameSamples(short * psSamples) {
		...
		case JB_BUFFERING_FRAME:
			...
			if (m_cEmptyCount < 5 && m_buffer.GetLastOutputFrame((LPBYTE)psSamples, cbFrame)) {
				double dfGainLinear = pow(10.0, -10.0 * m_cEmptyCount / 20.0);
				int cSamples = m_cbFrameSize / sizeof(short);
				for (int i = 0; i < cSamples; i++) {
					psSamples[i] = (short)(psSamples[i] * dfGainLinear);
				}
				m_cEmptyCount++;
			}
			else {
