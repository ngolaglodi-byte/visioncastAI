...
bool 
CMWDXRenderThread::open_render(
	mw_render_init_t* t_p_init,
	HWND t_h_wnd)
{
	bool t_b_ret = false;
	t_b_ret = m_p_render->initialize(
		t_p_init->m_u32_width,
		t_p_init->m_u32_height,
		t_p_init->m_mode,
		t_p_init->m_u8_need_rev,
        false,
		t_h_wnd,
		t_p_init->m_csp_in,
		t_p_init->m_clr_rng_in,
		t_p_init->m_clr_rng_out);
	if(!t_b_ret)
		return t_b_ret;
...
DWORD CMWDXRenderThread::render_thread_proc()
{
	...
	while(m_b_running){
		...
		if(t_u32_id==t_u32_id_render)
			...
			if(t_p_buf!=NULL){
				m_p_render->paint((unsigned char*)t_p_buf->m_p_buf);
			}
		}
	}
