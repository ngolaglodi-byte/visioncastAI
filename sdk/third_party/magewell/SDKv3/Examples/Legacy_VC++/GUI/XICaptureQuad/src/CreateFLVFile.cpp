...
inline void PrintErrorInfo(char* pStr, int ret)
{
	...
	char* pError = av_make_error_string(chError, AV_ERROR_MAX_STRING_SIZE, ret);
	fprintf(stderr, "%s: %s\n", pStr, pError);
}

BOOL CCreateFLVFile::Init(char* pFileName, int cx, int cy, DWORD dwFrameDuration)
{
	const char* filename;
	AVCodec* video_codec;
	int ret;
	...
	if ((ret = av_image_alloc(m_src_data, m_src_linesize,
		m_cx, m_cy, CAPTURE_PIX_FMT, 16)) < 0) {
	...
	if (m_outFmt->video_codec != AV_CODEC_ID_NONE) {
		add_stream(&m_stVideo, m_oc, &video_codec, m_outFmt->video_codec);
		m_nHaveVideo = 1;
		encode_video = 1;
	}
...
int CCreateFLVFile::WriteVideoFrame(unsigned char* pBuffer, int nStride/*, AVFormatContext *oc, OutputStream *ost*/)
{
	int ret;
	AVCodecContext *c;
	AVFrame *frame;
...
void CCreateFLVFile::open_video(AVFormatContext *oc, AVCodec *codec, OutputStream *ost, AVDictionary *opt_arg)
{
	int ret;
...
AVFrame* CCreateFLVFile::get_video_frame(OutputStream *ost, unsigned char* pBuffer, int nStride)
{
	AVCodecContext *c = ost->enc;
	...
	sws_scale(ost->sws_ctx,
		(const uint8_t * const *)m_src_data, m_src_linesize,
...
int CCreateFLVFile::write_frame(AVFormatContext *fmt_ctx, const AVRational *time_base, AVStream *st, AVPacket *pkt)
{
...
void CCreateFLVFile::log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt)
{
	...
	TRACE("pts:%s pts_time:%s dts:%s dts_time:%s duration:%s duration_time:%s stream_index:%d\n",
		av_ts_make_string(strTime, pkt->pts), av_ts_make_time_string(strTime1, pkt->pts, time_base),
		av_ts_make_string(strTime, pkt->dts), av_ts_make_time_string(strTime1, pkt->dts, time_base),
		av_ts_make_string(strTime, pkt->duration), av_ts_make_time_string(strTime1, pkt->duration, time_base),
		pkt->stream_index);
}

AVFrame* CCreateFLVFile::alloc_picture(enum AVPixelFormat pix_fmt, int width, int height)
{
	AVFrame *picture;
	int ret;
