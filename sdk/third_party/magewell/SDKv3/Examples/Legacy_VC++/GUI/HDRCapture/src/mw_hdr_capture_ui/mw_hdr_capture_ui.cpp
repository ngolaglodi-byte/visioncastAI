...
bool CMWHDRCaptureUI::setup_window(char* t_cs_name)
{
	...
	// s.1.2 create window
	s_p_window = glfwCreateWindow(DFT_WINDOW_WIDTH,DFT_WINDOW_HEIGHT,t_cs_name,NULL,NULL);
	if(s_p_window == NULL){
		MessageBoxA(NULL,
	...
	m_p_menubar->add_menu_item(m_cs_menu_device, "None - Only HDMI Supported");
	m_p_menubar->set_menu_item_checked(m_cs_menu_device,"None - Only HDMI Supported",true);
	if (m_p_capture == NULL) 
	...
	m_p_menubar->add_menu_item(m_cs_menu_hdr_setting,m_cs_menu_preview_auto);
	m_p_menubar->set_menu_item_checked(m_cs_menu_hdr_setting,m_cs_menu_preview_auto,true);
	m_p_menubar->add_menu_item(m_cs_menu_hdr_setting,m_cs_menu_preview_hdr);
	m_p_menubar->set_menu_item_checked(m_cs_menu_hdr_setting,m_cs_menu_preview_hdr,false);
	m_p_menubar->add_menu_item(m_cs_menu_hdr_setting,m_cs_menu_preview_sdr);
	m_p_menubar->set_menu_item_checked(m_cs_menu_hdr_setting,m_cs_menu_preview_sdr,false);

	m_p_hdr_param = new CMWHDRParam(m_p_ui_manager);
	m_p_hdr_param->set_param_change_callback(callback_hdr_value_change,this);

	m_p_menubar->add_menu_submenu(m_cs_menu_hdr_setting,m_cs_menu_hdr_param,m_p_hdr_param);
...
void CMWHDRCaptureUI::render_ui()
{
	...
	sprintf(t_cs_size,"%dGB %d MB %dKB %dB",
		t_n_gb,
		t_n_mb,
		t_n_kb,
		t_n_b);
...
void CMWHDRCaptureUI::callback_menu_item_selected_proc(
	char* t_cs_menu, 
	char* t_cs_menu_item, 
	int t_n_index)
{
...
void CMWHDRCaptureUI::callback_video_capture_proc(
	BYTE* pbFrame, 
	int cbFrame, 
	UINT64 u64TimeStamp)
{
	...
	while(true){
		...
		if(t_dw_ret == WAIT_OBJECT_0){
			...
			if(t_b_re){
				...
				memset(m_wcs_file_name,0,1024*sizeof(wchar_t));
				memset(m_wcs_file_name_main,0,1024*sizeof(wchar_t));
				memset(m_wcs_file_name_sub,0,1024*sizeof(wchar_t));
				memset(m_wcs_file_name_suffixes,0,1024*sizeof(wchar_t));
				m_n_file_name_index = 0;

				int t_n_len_file_name = wcslen(m_ofn.lpstrFile)*sizeof(wchar_t);
				memcpy(m_wcs_file_name,m_ofn.lpstrFile,t_n_len_file_name);
				memcpy(m_wcs_file_name_sub,m_wcs_file_name,t_n_len_file_name);
				...
				if (m_p_encoder != NULL) 
					if(t_gpu.platform == mw_venc_platform_t::MW_VENC_PLATFORM_AMD)
						t_b_encoding = false;
					else
...
void 
CMWHDRCaptureUI::callback_encode_proc(
	...
	mw_venc_frame_info_t *p_frame_info)
{
	...
	if(m_clock_start>0){
		...
		if(t_u64_now-m_clock_start>CLOCKS_PER_SEC*60*60){ // split file per hour
			if(m_p_file!=NULL){
				...
				if(p_frame_info->frame_type==MW_VENC_FRAME_TYPE_IDR){
					mw_mp4_close(m_p_file);
					m_p_file = NULL;
					m_clock_start = 0;
				}
	...
	if(m_p_file){
		...
		t_puc_buf = setup_for_hdr((unsigned char**)&p_frame,&frame_len);
		...
		mw_mp4_write_video(m_p_file, (const unsigned char*)p_frame, frame_len, p_frame_info->pts);
...
mw_render_fourcc_t CMWHDRCaptureUI::get_render_fmt_from_fourcc(DWORD t_dw_fourcc)
{
...
void 
CMWHDRCaptureUI::set_capture_status_text(
	char* t_cs_text,
...
void 
CMWHDRCaptureUI::set_encode_status_text(
	char* t_cs_text,
...
unsigned char* 
CMWHDRCaptureUI::setup_for_hdr(
	unsigned char **t_ppuc_stream_buf, 
	unsigned int *t_pui_stream_len)
{
	...
	bHaveSps = HevcFindSps(*t_ppuc_stream_buf, *t_pui_stream_len, &iSpsStart, &iSpsEnd);
	if(!bHaveSps){
		return NULL;
	}
	...
	HevcParseSPS(&stSpsInfo, *t_ppuc_stream_buf + iSpsStart + 5, iSpsEnd - iSpsStart - 5);
	...
	pucOutStream = (unsigned char *)malloc(*t_pui_stream_len - (iSpsEnd - iSpsStart - 5) + uiSpsLen + uiSeiLen);
	...
	memcpy(pucOutStream + uiSeiLen + iSpsStart + 5 + uiSpsLen, *t_ppuc_stream_buf + iSpsEnd, *t_pui_stream_len - iSpsEnd);
	*t_pui_stream_len = *t_pui_stream_len - iSpsEnd + uiSpsLen + iSpsStart + 5 + uiSeiLen;
