...
class CVideoCaptureThread(QThread):
    ...
    def video_capture_pro(self):
        ...
        malloc_size = t_frame_size
        if MWFOURCC_NV12 == self.m_fourcc:
            malloc_size = t_frame_size * 2
        
        t_buffer = []#create_string_buffer(t_frame_size)
        write_index = 0
        for i in range(4):
            buf = create_string_buffer(malloc_size)
            t_buffer.append(buf)
        ...
        while t_b_done == False:
            ...
            for i in range(4):
                self.m_lib_capture.mw_pin_video_buffer(self.m_h_channel,addressof(t_buffer[i]),t_frame_size)
            ...
            while self.m_b_running:
                ...
                if t_wait_ret == win32event.WAIT_OBJECT_0 + 0:
                    ...
                    t_ret = self.m_lib_capture.mw_capture_video_frame_to_virtual_address_ex(
                        self.m_h_channel,
                        t_video_buffer_info.iNewestBufferedFullFrame,
                        addressof(t_buffer[write_index]),
                        t_frame_size,
                        t_cb_stride,
                        False,
                        0,
                    ...
                    if self.m_callback != 0:
                        ...
                        self.m_callback(addressof(t_buffer[write_index]),t_size,t_video_frame_info.allFieldStartTimes[0])
            ...
            for i in range(4):
                self.m_lib_capture.mw_unpin_video_buffer(self.m_h_channel,addressof(t_buffer[i]))
...
class CAudioCaptureThread(QThread):
    ...
    def run(self):
        ...
        write_index = 0
        for i in range(16):
            buf = create_string_buffer(t_n_buf_size)
            t_buffer.append(buf)
        while t_b_done == False:
                            ...
                            if t_ret == MW_SUCCEEDED:
                                if self.m_callbcak != 0:
                                    for i in range(MWCAP_AUDIO_SAMPLES_PER_FRAME):
                                        t_pos_0 = i*8*4
                                        t_pos_1 = i*8*4+4*4
                                        t_buffer[write_index][t_n_buf_pos + 0] = t_audio_frame.adwSamples[t_pos_0+2]
                                        t_buffer[write_index][t_n_buf_pos + 1] = t_audio_frame.adwSamples[t_pos_0+3]
                                        t_buffer[write_index][t_n_buf_pos + 2] = t_audio_frame.adwSamples[t_pos_1+2]
                                        t_buffer[write_index][t_n_buf_pos + 3] = t_audio_frame.adwSamples[t_pos_1+3]
                                        t_n_buf_pos+=4
                                    self.m_lib_capture.mw_get_device_time(self.m_h_channel,t_ll_current)
                                    if t_n_buf_pos == t_n_buf_size:
                                        self.m_callbcak(addressof(t_buffer[write_index]),t_n_buf_size,t_ll_current.m_ll_device_time.value)
                                        t_n_buf_pos = 0
