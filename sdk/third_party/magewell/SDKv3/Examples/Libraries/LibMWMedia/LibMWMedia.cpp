	...
	)
{
	...
	if (!pRet->initialize(cx, cy, dwFourcc,bReverse, bMirror, hWnd, MW_CSP_BT_709, MW_CSP_LEVELS_TV, MW_CSP_LEVELS_PC)) {
		delete pRet;
		pRet = NULL;
	}
...
)
{
	...
	MWDXRender* pRet = (MWDXRender*)hRenderer;
	return pRet->get_max_latency(pMaxLatency);
}

BOOL
LIBMWMEDIA_API
MWD3DSetMaximumLatency(
	HD3DRENDERER			hRenderer,
	UINT					nMaxLatency
)
{
	...
	MWDXRender* pRet = (MWDXRender*)hRenderer;
	return pRet->set_max_latency(nMaxLatency);
}
	...
	)
{
	...
	do {
		...
		if (!pRender->Create(nSamplesPerSec, nChannels, nSamplesPerFrame, nBufferFrameCount))
			break;
...
)
{
	...
	return pRender->GetVolume(lpVolume);
}

BOOL
LIBMWMEDIA_API
MWDSoundRendererSetVolume(
HDSoundRENDERER hRenderer,
LONG lVolume
)
{
	...
	return pRender->SetVolume(lVolume);
}
